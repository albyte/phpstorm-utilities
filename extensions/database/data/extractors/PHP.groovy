...
def printPhp(level, col, o) {
  switch (o) {
    ...
    case Tuple: printPhp(level, o[0], o[1]); break
    case Map:
      OUT.append("[")
      o.entrySet().eachWithIndex { entry, i ->
        ...
        OUT.append("=> ")
        printPhp(level + 1, null, entry.getValue())
      }
    ...
    case Iterable:
      ...
      o.eachWithIndex { item, i ->
        ...
        printPhp(level + 1, null, item)
      }
      if (plain) OUT.append("]") else OUT.append("$NEWLINE${INDENT * level}]")
      break
    default:
      if (col != null) printPhp(level, null, FORMATTER.formatValue(o, col))
      else OUT.append("$o")
      break
  }
}

printPhp(0, null, ROWS.transform { row ->
